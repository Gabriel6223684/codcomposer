...
switch($action) {
    ...
    case 'list':
        $stmt = $pdo->query("SELECT id, nome, cpfcnpj, email FROM fornecedor"); // Não retorna senha
    ...
    case 'create':
        ...
        $stmt = $pdo->prepare("INSERT INTO fornecedor (nome, cpfcnpj, email, senha) VALUES (?, ?, ?, ?)");
    ...
    case 'update':
        ...
        if ($senha) {
            ...
            $senhaHash = password_hash($senha, PASSWORD_DEFAULT);
            $stmt = $pdo->prepare("UPDATE fornecedor SET nome=?, cpfcnpj=?, email=?, senha=? WHERE id=?");
            $stmt->execute([$nome, $cpfcnpj, $email, $senhaHash, $id]);
        } else {
            // Não altera a senha
            $stmt = $pdo->prepare("UPDATE fornecedor SET nome=?, cpfcnpj=?, email=? WHERE id=?");
